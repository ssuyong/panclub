...
$(document).ready(function(){
	...
	if(document.location.hash){
        ...
        //변수 재선언으로 info[4]에 매개변수 무효화되서 주석처리함    
       ...
       // console.log("sYmd:"+sYmd);
        ...
        findDataToServer("/order/pc-req-list",page);
        
        const params = new URL(document.location).searchParams;
		if( params.get('popup')=='open' && pcReqNo != '')
		{
					  $.fancybox.open({
	...
	$("#btnFind").click(function(){
		...
		findDataToServer("/order/pc-req-list", 1);
	});
...
// 칼럼 레이아웃 작성
var columnLayout = [ 
	 { dataField : "regYmd",    headerText : "요청일자", width : 80}
	,{ dataField : "regHmsg",   headerText : "요청시간", width : 80}	 
	,{ dataField : "procState",   headerText : "처리상태", width : 80}	  
	,{ dataField : "itemQty",   headerText : "아이템수" , width : 60}
	,{ dataField : "procCount",   headerText : "처리품목", width : 60}	 
	,{ dataField : "rejectCount",   headerText : "거부품목", width : 60}	 
	,{ dataField : "orderDelCount",   headerText : "주문삭제품목", width : 80}	 
	,{ dataField : "orderNo",   headerText : "처리번호", width: 120, headerTooltip : {
        show : true,
        tooltipHtml : '외부업체의 경우 생성된 주문번호입니다'
	    } ,
	     renderer : {
	            type : "TemplateRenderer" 
	     },
	     labelFunction : function (rowIndex, columnIndex, value, headerText, item ) { // 처리번호가 여러개인 경우도 있으므로 탬플릿렌더러를 이용해서 하이퍼링크로 복수의 주문번호로 연결 가능하도록
	     		if(value == null )	return '';
	     		
	     		const orderNumbers = value.split(',');
	     		let returnHtml = '';
	     		for(orderNo of orderNumbers)
	     		{
					if(returnHtml != '') returnHtml += '<a>,</a>'
				  
					returnHtml += '<a class="auigrid-color-style-link" style="color: black; ;" href="/order/order-list#info1!'+orderNo+'!'+$("#startpicker-input").val()+'!'+$("#endpicker-input").val()+'!Y!!!"" target="_blank">' + orderNo + '</a>'
				} 
	...
	,{ dataField : "pcReqNo",   headerText : "요청번호", width: 120,
...
//	,{ dataField : "procStep",      headerText : "접수단계"  , width : 120   }
	,{ dataField : "gvComCode",     headerText : "요청업체코드", width : 80     }
	,{ dataField : "gvCustName",     headerText : "요청업체명", width : 120, style:"left"}
	
	,{ dataField : "gvPlacNo",   headerText : "요청업체 발주번호" , width : 120}
	...
	,{ dataField : "senderCustName",      headerText : "보내는거래처", width : 100, style:"left"}
	,{ dataField : "senderName",      headerText : "보내는사람", width : 100, style:"left"}
	,{ dataField : "senderTel",      headerText : "보내는이연락처", width : 100}
	,{ dataField : "senderAddr1",      headerText : "보내는이주소", width : 100, style:"left"}
	,{ dataField : "receiverCustName",      headerText : "받는거래처", width : 100, style:"left"}
	,{ dataField : "receiverName",      headerText : "받는사람", width : 100, style:"left"}
	,{ dataField : "receiverTel",      headerText : "받는이연락처", width : 100}									
	,{ dataField : "receiverAddr1",      headerText : "받는이주소", width : 100, style:"left"}		
		
	,{ dataField : "gvMemo",      headerText : "요청메모"   , style:"left", width : 120  }
	...
	,{ dataField : "inMemo1",      headerText : "기타메모" , width : 120  , style:"left" }
	,{ dataField : "regUserName",      headerText : "등록자", width : 100,visible: false}
...
const footerLayout = [{
	...
	dataField: "orderDelCount",
	positionField: "orderDelCount",
	operation: "SUM"  
...
function createAUIGrid(columnLayout) {
	...
	var auiGridProps = {		
			...
			//처리는 완료인데 주문등록이 누락되었거나 해서 아이템수가 처리품목+거부품목보다 높을경우 행자체가 눈에 들어오도록 표시
			rowStyleFunction: function (rowIndex, item) { 
				if (item.procState=='완료' && (item.itemQty > (item.procCount + parseInt(item.rejectCount) + item.orderDelCount))) 
				{	
					return "custom-font-bold";	
				}	
				return null;	
			}
	};
			...
			if (data.pcReqList.length == 0){
				...
				for(i=0;i<data.pcReqList.length;i++){
					list.push({
						...
						,orderNo : data.pcReqList[i].orderNo
						,rcvlogisCode : data.pcReqList[i].rcvlogisCode
						,rejectCount : data.pcReqList[i].rejectCount
						,procCount : data.pcReqList[i].procCount
						,orderDelCount : data.pcReqList[i].orderDelCount
					});
