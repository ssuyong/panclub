...
$(document).ready(function(){
	...
	//$("#btnRegTaxBill").prop("disabled", true);
	

    	
	//$("#taxBillRegYN").val("");
	//$("#taxBillRegYN").prop("disabled", true);
	...
	if(document.location.hash){
        ...
        if ( typeof taxBillRegYN == 'undefined'){ taxBillRegYN = ''	}
		...
		if(ledgType == "판매출고"){
			$("#taxBillRegYN").prop("disabled", false);			
		}
        ...
        findDataToServer("/biz/transaction-list",page);
  		...
  		if($("#custCode").val() !== '' && $("#ledgType").val() === '판매출고' && $("#taxBillRegYN").val() === 'N'){
		 	$("#btnRegTaxBill").prop("disabled", false);
		 }else{
		 	$("#btnRegTaxBill").prop("disabled", true);
	 	} 
	...
	$("#btnFind").click(function(){
		...
		var taxBillRegYN_val =$("#taxBillRegYN").val();
		var mainYN_val = ($('#mainYN').is(':checked') ? "Y" : "N");
				
		document.location.hash = '#info'+currentPage+"!"+custCode_val+"!"+sYmd+"!"+eYmd+"!"+ymdIgnoreYN
												+"!"+ledgType_val+"!"+orderGroupId_val+"!"+carNo_val+"!"+itemId_val+"!"+itemNo_val+"!"+srCode_val+"!"+clType_val+"!"+taxBillRegYN_val+"!"+mainYN_val;
		 findDataToServer("/biz/transaction-list",page);
		//findDataToServer("/biz/cust-ledg", 1);
		// findCustMst('/base/cust-list');
	});
	
	$("#ledgType").change(function() {
	...
	{  headerText : "기본정보", 
     ...
     {  headerText : "부품", 
		children: [
        	...
        	{ dataField: "costPrice",         headerText: "입고단가", width : 86, dataType: "numeric", formatString: "#,##0", style: "right" , editable: false   },
        	{ dataField: "unitPrice",         headerText: "출고단가", width : 86, dataType: "numeric", formatString: "#,##0", style: "right" , editable: false },
        	...
        	{ dataField: "clType",      headerText: "일반/보험", width : 68, editable: false }  ,
        	...
        	{ dataField : "srCode",      headerText : "관리그룹"  , width : 80   } //2023.10.16 bk
...
function findDataToServer(url,page) {
	...
	var taxBillRegYN = $("#taxBillRegYN").val(); //240201 yoonsang
			...
			if (data.transactionList.length == 0){
				...
				for(i=0;i<data.transactionList.length;i++){
					list.push({
						...
						,summary2: data.transactionList[i].summary2	//240116 yoosang
...
function taxBillReg() {
	...
	for (let i =0, len = checkedItems.length; i < len; i++) {
		rowItem = checkedItems[i];
		summary = summary + "%5E" + rowItem.item.summary;
		seq = seq + "%5E" + rowItem.item.seq;
		custCode2 = checkedItems[0].item.custCode;
					
		clAmt = clAmt+ rowItem.item.sumPriceTax;
		
		if (rowItem.item.custCode != custCode2) {
	...
	$.fancybox.open({
		href: '/base/taxBill-up?custCode2=' + custCode2+ '&clAmt=' + clAmt + '&summary=' + summary + '&seq=' + seq,
		type: 'iframe',
		width: '60%',
		height: '80%',
		padding: 0,
...
function getData(selectedValue){
	...
	}else if(selectedValue == "판매출고"){
		
		$("#taxBillRegYN").val("");
		$("#taxBillRegYN").prop("disabled", false);	
	}else if(selectedValue == "반입"){
