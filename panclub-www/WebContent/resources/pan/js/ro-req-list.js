...
$(document).ready(function(){
	...
	$("#btnFind").click(function(){
		...
		document.location.hash = '#info'+currentPage+"!"+receiver_val+"!"+sYmd+"!"+eYmd;
		
		findDataToServer("/logis/ro-req-list", 1);
	});
...
var columnLayout = [ 
	...
	,{ dataField : "dmdTime",   headerText : "구분", width: 70} 
	,{ dataField : "roReqNo",     headerText : "요청번호", width : 120 ,
	...
	,{ dataField : "custName",   headerText : "주문처명" , width: 200 }
	,{ dataField : "carNo",   headerText : "차량정보" , width : 90  }
	,{ dataField : "roSumPrice",     headerText : "공급가액", dataType: "numeric",formatString: "#,##0"  , style:"right" , width : 150  }
	,{ dataField : "roTaxPrice",     headerText : "세액" , dataType: "numeric",formatString: "#,##0"  , style:"right", width : 150  }
	,{ dataField : "roSumPriceTax",     headerText : "합계" , dataType: "numeric",formatString: "#,##0"  , style:"right", width : 150  }
	...
	,{ dataField : "attaFileOri",   headerText : "첨부파일", width : 90 
		/*
		,style: "myLinkStyle-left",
		renderer: {
			type: "LinkRenderer",
			linkField: "attaFile",
			//baseUrl: "https://img.panclub.co.kr/" + item.comCode +"/wd/" 
			//,target: "_blank"
			...
			//jsCallback: function (rowIndex, columnIndex, value, item) {
				...
				baseUrl: "javascript",
				 jsCallback: function(rowIndex, columnIndex, value, item) {
      	  		var comCode = item.comCode;
       	 		var url = "https://img.panclub.co.kr/" + comCode + "/wd/" + item.attaFile;
				 window.open(url)			
				}			
			}
			*/	
	...
	,{ dataField : "roWay",   headerText : "반출방법" , width: 90 }
	,{ dataField : "regUserId",      headerText : "요청자", width : 90 }
	,{ dataField : "receiver",     headerText : "수신자", width : 90  }
	,{ dataField : "procStatus",      headerText : "처리내역" , width : 90 }
	,{ dataField : "gvComCode",   headerText : "요청업체코드" , width: 100 }// 프로시저에서 반환받아서 쓰세요 ,visible: false
...
var footerLayout = [
		...
		{		dataField: "roTaxPrice",		positionField: "roTaxPrice",		operation: "SUM",		formatString: "#,##0"	}, 
		{		dataField: "roSumPriceTax",		positionField: "roSumPriceTax",		operation: "SUM",		formatString: "#,##0"	}, 
...
function createAUIGrid(columnLayout) {
	...
	AUIGrid.setFooter(myGridID, footerLayout); //20230710 bk
	// 에디팅 시작 이벤트 바인딩
	...
	AUIGrid.bind(myGridID, "cellDoubleClick", function (event) {
		...
		var roReqNo = event.item.roReqNo;
		//console.log("storageUseReqNo:"+storageUseReqNo);  
		let gvComCode = event.item.gvComCode;
		if (event.dataField == 'roReqNo') {   
			$.fancybox.open({
			  href : '/logis/ro-req-item-list?roReqNo='+roReqNo + '&gvComCode=' + gvComCode    , // 불러 올 주소
			  type : 'iframe',
			  width : '90%',
			  height : '90%',
			  padding :0,
		...
		success:function(data){
			...
			if (data.reqList.length == 0){
				...
				for(i=0;i<data.reqList.length;i++){
					...
					//console.log("data.reqList[i].attaFileOri : "+ data.reqList[i].attaFileOri)
					//console.log("attaYN : "+ attaYN)
					...
					list.push({
						...
						,roSumPrice: data.reqList[i].roSumPrice //20230710 bk
						,roTaxPrice: data.reqList[i].roTaxPrice //20230710 bk
						,roSumPriceTax: data.reqList[i].roSumPriceTax //20230710 bk
						,gvComCode :data.reqList[i].gvComCode  
