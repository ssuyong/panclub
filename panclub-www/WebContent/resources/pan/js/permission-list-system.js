...
const TAB_LIST = { 
					 'TAB_PERMISSION' : {
						index : 1	,
						leftGridID : '#grid_wrap_permission',
						leftFind : ()=>{
							createAUIGrid_permission();
							permissionListfind(''); 
						},
						dataUpdate :()=>{
							permissionListAdd();
					 ...
					 'TAB_CUST' : {
						index : 3   ,
						leftGridID : '#grid_wrap_cust' , 
						rightGridID : '#grid_wrap_custPermission',
						selectTextId : '#selectCust',
						templateClassName : '.templateSelect.custTemplate' ,
						leftFind : ()=>{
							createAUIGrid_cust();
							permissionCustList();
...
//					  },
					 'TAB_PERMISSION_CUST' : {
						index : 5 , 
						leftGridID : '#grid_wrap_pcc' ,
						rightGridID : '#grid_wrap_pccPermission',
						selectTextId : '#selectPcc',
						leftFind : ()=>{
							createAUIGrid_pcc();
							permissionListfind('pcc');  
						},
						dataUpdate :()=>{
							permissionPccSet();
						},
						rightFind : (permissionIdx)=>{
							permissionCustList(permissionIdx);
						}
					},
//					 'TAB_PERMISSION_USER' : {
//						index : 6 , 
...
function btnSave()
{ 
	if(getTabName() != 'TAB_PERMISSION' && !confirm('저장하시겠습니까?'))
	{
		return;
	} 
		TabDataUpdate();
...
//그리드가 많아서 한번에 만들면 페이지 열릴때 오래걸려서 탭을 누를때 생성(1번만)하는것으로 수정

//그리드 셋팅-권한설정
function createAUIGrid_permission() {
	
	if(TAB_LIST.TAB_PERMISSION.GridGen) return; 
	const permissionColumnLayout = [
		...
		,{ dataField : "type",   headerText : "권한타입", width : 80} 
		,{ dataField : "url",   headerText : "URL", width : 200, style : "left" } 
	 ...
	 //권한설정 그리드
	 AUIGrid.create( TAB_LIST.TAB_PERMISSION.leftGridID , permissionColumnLayout, auiGridProps);
	 
	 TAB_LIST.TAB_PERMISSION.GridGen = true;
}
//그리드 셋팅-템플릿
...
//그리드 셋팅 - 업체별권한
function createAUIGrid_cust() {
	
	if(TAB_LIST.TAB_CUST.GridGen) return;
	
	const custColumnLayout = [
		{ dataField : "custCode",    headerText : "업체코드",width : 100 , editable: false} 
	...
	const custPermissionColumnLayout = [
	 ...
	 //업체별권한 그리드
	 AUIGrid.create( TAB_LIST.TAB_CUST.leftGridID , custColumnLayout, {...auiGridProps , rowIdField : 'custCode'});
	 AUIGrid.create( TAB_LIST.TAB_CUST.rightGridID ,custPermissionColumnLayout, auiGridProps);


	AUIGrid.bind(TAB_LIST.TAB_CUST.leftGridID, "cellClick", (event)=>{
	 
		$(getSelectTextId()).text(`${event.item.custName}[${event.item.custCode}]`);
		permissionCustDetail(event.item.custCode)
	...
	TAB_LIST.TAB_CUST.GridGen = true;
}
//그리드 셋팅 - 권한별업체
function createAUIGrid_pcc() {
	
	if(TAB_LIST.TAB_PERMISSION_CUST.GridGen) return;
	 
	const pccColumnLayout = [
		{ dataField : "idx",    headerText : "idx", visible : false} 
		,{ dataField : "code",    headerText : "권한코드", width : 80} 
		,{ dataField : "name",   headerText : "권한명", width : 160} 
		,{ dataField : "type",   headerText : "권한타입", width : 60} 
		,{ dataField : "pccCount",   headerText : "보유권한업체수", width : 100} 
		,{ dataField : "url",   headerText : "URL", width : 200, style : "left", visible : false }  
		,{ dataField : "memo",   headerText : "메모", width : 400, style : "left" } 
		
	];
	const pccDetailColumnLayout = [ 
		{ dataField : "custCode",    headerText : "업체코드", width : 120} 
	 ...
	 //권한별업체 그리드
	AUIGrid.create( TAB_LIST.TAB_PERMISSION_CUST.leftGridID , pccColumnLayout, auiGridProps);
	AUIGrid.create( TAB_LIST.TAB_PERMISSION_CUST.rightGridID , pccDetailColumnLayout, {...auiGridProps , rowIdField : 'custCode'});
	

	AUIGrid.bind(TAB_LIST.TAB_PERMISSION_CUST.leftGridID, "cellClick", (event)=>{
		 
		const selectPcc = $(getSelectTextId());
		selectPcc.text(event.item.name);
		selectPcc.attr('index' , event.item.idx);
		permissionCustList(event.item.idx); 
	});
	
	TAB_LIST.TAB_PERMISSION_CUST.GridGen = true;
...
//권한 조회
function permissionListfind(type)
{
	$.ajax(	{
		type : "POST",
...
{
	const dataArr = [
					...AUIGrid.getAddedRowItems(TAB_LIST.TAB_PERMISSION.leftGridID).map((row)=>{return {permission:{...row , idx :-1 , valid : row.valid || false , hiddenMenu : row.hiddenMenu || false} , workingType : 'ADD'}}) , 
				 	...AUIGrid.getEditedRowItems(TAB_LIST.TAB_PERMISSION.leftGridID).map((row)=>{return {permission:{...row} , workingType : 'EDITE'}}) , 
				  	...AUIGrid.getRemovedItems(TAB_LIST.TAB_PERMISSION.leftGridID).map((row)=>{return {permission:{idx : row.idx} , workingType : 'REMOVE'}})  
				 ]; 
	const addNullDataChk = dataArr.filter((row)=>row.workingType=='ADD' && (((row.permission.code ?? '' ) == '') || ((row.permission.name ?? '')  == '') || ((row.permission.type ?? '')  == '') ))
	if(addNullDataChk.length>0)
	{
		
		alert(`추가된 권한은 코드,이름,타입이 필수입니다`); 
		return;
	}		 
	if(dataArr.length == 0 )
	{
	...
	ProgressManager.open(dataArr.length);
 	permissionAdd(dataArr ,0)
}
//권한설정 - 추가삭제수정 처리
function permissionAdd(dataArr , index)
{
	...
	{	 
		ProgressManager.end('처리성공' , ()=>{
											TAB_LIST.TAB_PERMISSION.leftFind();
										});
	}
}
...
function permissionTemplateListfind()
{
	$.ajax(	{
		...
		contentType : "application/x-www-form-urlencoded;charset=UTF-8",
		success:function(result){  
		 	AUIGrid.setGridData(TAB_LIST.TAB_TEMPLATE.leftGridID , result.filter(r=>r.templateType=='cust'));
...
function permissionTemplateDetailfind(idx)
{
	$.ajax(	{
		...
		contentType : "application/x-www-form-urlencoded;charset=UTF-8",
		success:function(result){   
			const resultData = result.map(row=>{return {...row  , templateIdx : idx}}).sort((a,b)=>a.code.localeCompare(b.code));
...
{
	const dataArr = [
					...AUIGrid.getAddedRowItems(TAB_LIST.TAB_TEMPLATE.leftGridID).map((row)=>{return {template:{...row , idx :-1, templateType : 'cust' } , workingType : 'ADD'}}) , 
				 	...AUIGrid.getEditedRowItems(TAB_LIST.TAB_TEMPLATE.leftGridID).map((row)=>{return {template:{...row, templateType : 'cust'} , workingType : 'EDITE'}}) , 
...
/*********************************************업체 권한 설정***********************************/
//업체 조회 (권한별 업체조회때 매개변수로 조회)
function permissionCustList(selectIndex = -1)
{

	$.ajax(	{
		type : "POST",
		url : '/base/permission-cust-list',
		dataType : "json", 
		async: false,
		data:{
			selectIndex
...
//업체가 가진 권한 조회
function permissionCustDetail(custCode)
{ 
	$.ajax(	{
		type : "POST",
		url : '/base/permission-cust-detail',
		dataType : "json", 
		async: false,
		data : {
			custCode
		...
		contentType : "application/x-www-form-urlencoded;charset=UTF-8",
		success:function(result){   
			const gridId = getTabRightGridId();
		 	AUIGrid.setGridData(gridId , result.map(row=>{return {...row  }}).sort((a,b)=>a.code.localeCompare(b.code)) );
			AUIGrid.resize(gridId);
		},
		error:function(x,e){
			errFun(x,e);
	...
	$.ajax(	{
		...
		success:function(result){    
			...
			if(templateSelectClassName == '') return;
		 	...
		 	const optionHtml = result.filter(r=>r.templateType=='cust')
		 							 .reduce((a,c)=>{
...
$(".templateSelect").change(function(e){
	...
	$.ajax(	{
		...
		success:function(result){   
			...
			if(getTabName() == 'TAB_CUST')
			{
				const resultDataIdx = result.map(row=>row.idx); 
				const originData = AUIGrid.getGridData(getTabRightGridId());
 
				 
			 	const data = [... resultData, 
			 				  ... originData.filter(row=>!resultDataIdx.includes(row.idx)).map(row=>{return {...row,templateValid : false , valid:false}}) ].sort((a,b)=>a.code.localeCompare(b.code));
			 	AUIGrid.setGridData(permissionGridName , data);
				
			}
			else if(getTabName() == 'TAB_USER')
			{
...
//업체별 권한에서 업체에 담긴 권한 수정시 수정하려는 정보를 객체로 가공시켜줌 -> 처리는 permissionCustAdd
function permissionCustSet()
{
	const selectCustText = $(getSelectTextId()).text();
	if(selectCustText == '없음') 
	{
		alert('선택된 업체가 없습니다');
		return;
	}
	...
	const selectCustCode = selectCustText.split('[')[1].replace(']','');
	...
	//변경된 권한 정보를 가공
	const data = AUIGrid.getGridData(TAB_LIST.TAB_CUST.rightGridID).filter(row=>row.templateValid != row.valid).map(row=>{
																														return {
																																	workingType : 'PERMISSION_CHANGE',
																																	permissionIdx:row.idx , 
																																	valid:row.valid ,
																																	custCode : selectCustCode
																																}
																													});
	//템플릿 정보를 data맨앞에 삽입
	data.unshift({workingType : 'TEMPLATE_SET' , templateIdx  , custCode : selectCustCode })
	 
	ProgressManager.open(data.length);
	permissionCustAdd(data, 0);
}
//업체 권한 수정 실제 처리
function permissionCustAdd(dataArr , index)
{
	ProgressManager.next();
	...
	if(dataArr.length > index)
	{
		...
		setTimeout(()=>{
			$.ajax(	{
				...
				//contentType : "application/x-www-form-urlencoded;charset=UTF-8",
				success:function(result){ 
					permissionCustAdd(dataArr , index+1)
				},
				error:function(x,e){
					errFun(x,e);
				}
	...
	{	 
		ProgressManager.end('처리성공' , ()=>{
											...
											if(tabName == 'TAB_CUST') 
												TabRightFind(dataArr[0].custCode); 
											else if(tabName == 'TAB_PERMISSION_CUST')
...
//권한별 업체에서 저장 
function permissionPccSet()
{
	
	
	const pccIndex = $(getSelectTextId()).attr('index');
	if($("#selectPcc").text() == '없음')
	{
		alert('권한을 선택해주세요');
		return;
	}

	const data = AUIGrid.getEditedRowItems("#grid_wrap_pccPermission").map(row=>{
																					return {
																								workingType : 'PERMISSION_CHANGE',
																								permissionIdx:pccIndex , 
																								valid:row.valid ,
