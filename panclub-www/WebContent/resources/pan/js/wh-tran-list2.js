...
$(document).ready(function(){
	...
	$("#btnFind").click(function(){
		...
		var branchComCode = $("#branchComCode").val(); 
				
		document.location.hash = '#info'+currentPage+"!"+custCode_val+"!"+sYmd+"!"+eYmd+"!"+branchComCode;
		 findDataToServer("/biz/transaction-list2",page);
...
var columnLayout = [ 
	{  headerText : "기본정보", 
		children: [
			...
			{ dataField: "branchComCode",        headerText: "지점코드"      , width: 50 , style : "left", visible: false }, 
			{ dataField: "branchName",        headerText: "지점명"      , width: 60 , style : "left"}, 
        	{ dataField: "summary",      headerText: "적요" ,      width: 86  , editable: false    
		...
		success:function(data){
			...
			if (data.transactionList.length == 0){
				...
				for(i=0;i<data.transactionList.length;i++){
					if(data.transactionList[i].branchComCode == "ㄱ000"){
						branchName = "P"
					}else if(data.transactionList[i].branchComCode == "ㅇ413"){
						branchName = "임파츠"
					}else if(data.transactionList[i].branchComCode == "ㅇ434"){
						branchName = "에스제이"
					}else if(data.transactionList[i].branchComCode == "ㅇ436"){
						branchName = "에스디"
					}else if(data.transactionList[i].branchComCode == "ㅋ127"){
						branchName = "케이에스"
					}else {
						branchName = ""
					}
					list.push({
						...
						,branchComCode: data.transactionList[i].branchComCode
