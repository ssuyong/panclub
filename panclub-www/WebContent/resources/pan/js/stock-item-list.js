...
var columnLayout = [ 
   ...
   /* 
 	...
 	var footerLayout = [{
		...
		dataField: "workableQty",
		positionField: "workableQty",
		operation: "SUM", 
		style:"right"
...
function findDataToServer(url,page) {
	...
	*/
	...
	var noRealYN = $('input:checkbox[name=noRealYN]').is(':checked'); 
	 ...
	 var qtyZeroYN = $('input:checkbox[name=qtyZeroYN]').is(':checked'); 
	 if(qtyZeroYN == true){
	  	qtyZeroYN = "Y";
	 }else{
	  	qtyZeroYN = "N";
	...
	//조회하려는 수탁업체 코드
	const consignCustCode = $("#consignCustCode")?.val().replaceAll(')','').split('(').reverse()[0] || '' ;
...
//						,qtyxPrice: (data.stockItemList[i].workableQty)*(data.stockItemList[i].centerPrice)
				...
				if(qtyZeroYN == 'Y') list = list.filter(r=>r.stockQty >0);
				 AUIGrid.setGridData("#grid_wrap", list);		
				// AUIGrid.setGridData("#grid_wrap", list.filter(r=>r.stockQty>0));
...
function consignComCodeList()
{
	$.ajax({
		...
		success: function(result) {
			const consingCustList = result.c_custList.filter(r=>r.validYN =='Y')
													 .map(r=>{
																return {custCode : r.custCode , custName : r.custName };
			                                             })
			                                             
			//consignCustCode
			const selectBox = $("#consignCustCode");
			selectBox.empty();
			selectBox.append(`<option></option>`)
			for(item of consingCustList)
			{
				selectBox.append(`<option>${item.custName}(${item.custCode})</option>`)
			}
