...
function setGridData(yymm , dateType) { //월간의 경우 'yyyy-' 포멧으로 담기고 일간의 경우 'yyyy-mm-' 포멧으로 담김
	...
	$.ajax({
		...
		success: function(data) {    
			...
			if(data.sIncReport.length == 0) return; // 데이터 길이가 0인경우도 종료
			if(dateType != DATETYPE.MONTH && dateType != DATETYPE.YEAR) return; //타입이 DATETYPE에 없는경우에도 종료
			 
			row.stdYmd = data.sIncReport[0].stdYmd.substr((dateType==DATETYPE.MONTH?8:5),2); // 일간의 경우 날짜에 일일이 들어가고 월간의 경우 월이 들어감 포멧으로 보면 일간 = 'dd' 월간 = 'mm'
 
			for(let i = 0 ; i < data.sIncReport.length ; i++)
			{
				let sIncReport = data.sIncReport[i];  //접근하기 편하도록 보관하는 변수
				if(row.stdYmd != sIncReport.stdYmd.substr((dateType==DATETYPE.MONTH?8:5),2)) // 날짜로 정렬된 리스트의 날짜가 변경되면 기존 데이터를 list에 담고 행데이터를 초기화
				{
					totalSum(row); 
					list.push(row);
					row = [];
					row.stdYmd = sIncReport.stdYmd.substr((dateType==DATETYPE.MONTH?8:5),2);
				}
				if(dateType == DATETYPE.MONTH || !row[sIncReport.category+"CC"+sIncReport.comCode.substring(1,4)]) //일일의 경우엔 그냥 금액을 넣고, 월간이지만 데이터가 아직 안담긴 경우 금액을 넣어서 초기화 시킴
					row[sIncReport.category+"CC"+sIncReport.comCode.substring(1,4)] = sIncReport.sumPriceTax; // 데이터가 담길곳에 금액을 담음 
				else if(dateType == DATETYPE.YEAR)
					row[sIncReport.category+"CC"+sIncReport.comCode.substring(1,4)] += sIncReport.sumPriceTax; //데이터가 이미 있고 연간인경우 금액을 sum함
...
{ // 전역변수 방지용 중괄호 
	let children = []; // 
	const childrenTexts = ['출고','입고','주문'] // 컬럼의 최상위 헤더 텍스트
	const code = ['000' , '413' , '434' , '436' , '127'] // 컴코드 컬럼 속성값
	const codeText = ['P','임파츠','SJ','SD','KS']; // 컴코드 커럼 텍스트
	for(let i = 1 ; i <4 ; i++)
	{ 
