...
carNo.addEventListener("blur", e => {
	$("#existEstiNoti").css("display","inline");
});
*/
...
$(document).ready(function(){
	...
	*/	
	...
	$("#btnUpt").click(function(){
		...
		//fn_delBlankRowAll(myGridID, "itemId");  //itemId가 공백인것은 상태값 초기화하여 저장안되게 처리.
		fn_dcProc();
		updateDataToServer("/order/estiAdd", "UPT");
	});
	$("#btnDel").click(function(){
		//alert("등록버튼");
		if (confirm("삭제되면 복구가 불가능 합니다. 견적 전체를 삭제하시겠습니까?\n품목삭제(행삭제)를 하려면 [수정]버튼을 클릭하세요.")){
			updateDataToServer("/order/estiAdd", "DEL");
		}
	});
	$("#btnNew").click(function(){
			 window.location.href = "/order/esti-up";
	});
	...
	$("input:radio[name=dcDspType]").click(function(){
        ...
        fn_dcProc();
        /*    
	    if($("input:radio[name=dcDspType]:checked").val()=='select'){
...
function createAUIGrid() {
	...
	var specialNoteList = ["", "국내재고없음"];

	// AUIGrid 칼럼 설정
	var columnLayout = [		 
		 { dataField : "idx",      headerText : "idx", width : 50, editable : false, visible : false }
		 
		,{ dataField : "dspNo",      headerText : "노출<br>순서", width : 70 , style : "auigrid-opt-col-style"}
		,{ dataField : "estiSeq",      headerText : "견적<br>순번", width : 70, editable : false, dataType: "numeric" }
		...
		//,{ dataField : "itemNameEn", headerText : "영문품명", width: 120, editable : false, style:"left"   }
		,{ dataField: "dcExceptYN", headerText: "할인제외", width: 56, editable: false , visible : false}
		,{ dataField : "cnt",      headerText : "수량*", width : 120, dataType: "numeric",formatString: "#,##0"  , style:"right  auigrid-must-col-style"
		...
		//,{ dataField : "supplyPrice",     headerText : "납품단가", dataType: "numeric",formatString: "#,##0" }
		//,{ dataField : "supplySumPrice",     headerText : "납품액" , dataType: "numeric",formatString: "#,##0"}
		...
		, {
			headerText: "발주처정보",
			children: [
			{ dataField : "placeCustCode",     headerText : "코드"  ,editable : true , style:" auigrid-opt-col-style" }
		...
		,{
		   dataField: "specialNote", headerText: "특이사항", editable: true, width:100
		  	,renderer: {
				type: "DropDownListRenderer",
				list: specialNoteList
			}
		}
	...
	*/
	...
	AUIGrid.bind(myGridID, "addRowFinish", function( event ) {
    	...
    	//AUIGrid.removeRow(myGridID, event.items.length+1);
	...
	AUIGrid.bind(myGridID, "pasteBegin", function(event) {
		...
		//if (event.dataField == 'itemNo') {
			/*
		var clipSplit  = event.clipboardData.split('\n');
		for (var i = 0, len = clipSplit .length; i < len; i++) {
	...
	AUIGrid.bind(myGridID, "pasteEnd", function(event) {
		...
		for (var i = 0, len = allItems .length; i < len; i++) {
			...
			placeCustCode = AUIGrid.getCellValue(myGridID, i, "placeCustCode");
			placeCustName = AUIGrid.getCellValue(myGridID, i, "placeCustName");
			
			
			//if (itemId != "" && itemId != 0 && typeof itemId != "undefined" && itemId != null){  //상품ID 존재하는 경우만
			//if ( (itemNo === undefined || itemNo == '') && (itemId === undefined || itemId == '') ){  //상품ID 존재하는 경우만
			//if ( itemNo !==undefined && (itemId === undefined || itemId == '') ){  //상품ID 존재하는 경우만. 이게 원본
	...
	AUIGrid.bind(myGridID, "rowAllChkClick", function (event) {
		if (event.checked) {
			// name 의 값들 얻기
			var uniqueValues = AUIGrid.getColumnDistinctValues(event.pid, "orderNo");
			// Anna 제거하기
			uniqueValues.splice(!uniqueValues.indexOf(""), 2);
			AUIGrid.setCheckedRowsByValue(event.pid, "orderNo", uniqueValues);
		} else {
			AUIGrid.setCheckedRowsByValue(event.pid, "orderNo", []);
		}
	});*/
    ...
    if (event.checked) {
        var uniqueValues = AUIGrid.getColumnDistinctValues(event.pid, "orderNo");       
        var emptyValues = uniqueValues.filter(function(value) {
            return value == "";
        });
        AUIGrid.setCheckedRowsByValue(event.pid, "orderNo", emptyValues);
    } else {
        AUIGrid.setCheckedRowsByValue(event.pid, "orderNo", []);
    }
});
...
function auiCellEditingHandler(event) { 
	...
	if (event.dataField == 'itemNo'){
		...
		//console.log ("event.rowIndex222222"+event.rowIndex);
	    fn_dcProc();
	...
	for (var i = 0, len = allItems .length; i < len; i++) {
		...
		if ( itemId !== undefined ){  //상품ID 존재하는 경우만
			AUIGrid.removeRow(myGridID, i+2);
		}
	}
	*/
...
function updateDataToServer( url, workingType ) {
	/*
	 ...
	 var sumPrice_ko = _cf_numToKor(sumPrice.toString());
	 
	 	 
	 var vatVal = _cf_vat('Y',sumPrice);
	...
	*/
	...
	if (workingType == 'UPT') { //수정인 경우 주문순번에 값이 없으면 기존 디테일 전제 삭제후 등록
		...
		//console.log("allItemDelYN:"+allItemDelYN);
	...
	//var isValid1 = AUIGrid.validateGridData(myGridID, ["itemNo", "salePrice", "cnt"], "품번, 수량, 견적단가 필드는 반드시 값을 입력해야 합니다.");
	//var isValidChanged1 = AUIGrid.validateChangedGridData(myGridID, ["itemNo", "cnt", "salePrice"], "품번, 수량, 견적단가 필드는 반드시 유효한 값을 입력해야 합니다.");
...
function findEsti(url) {
		...
		success:function(data){
			...
			if (data.estiList.length == 0){
				...
				for(i=0;i<data.estiList.length;i++){
    				...
    				sumPriceKor = data.estiList[i].sumPriceKor;    

    				branchCode  = data.estiList[i].branchCode;
    				regUserName  = data.estiList[i].regUserName;
    				
    				if (data.estiList[i].estiType == 1) {
					  document.querySelector('input[name="estiType"][value="1"]').checked = true;
					    click_EstiType(1);
					} else if (data.estiList[i].estiType == 2) {
					  document.querySelector('input[name="estiType"][value="2"]').checked = true;
					  click_EstiType(2);
					}
   
   /*
   					if (data.estiList[i].dcDspType == 1) {
					  document.querySelector('input[name="dcDspType"][value="1"]').checked = true;
					} else if (data.estiList[i].dcDspType == 2) {
  ...
  */
					...
					document.getElementById('btnOrder').classList.toggle('disabled');			
...
function findEstiItem(url) {
	...
	$.ajax(	{
		...
		success:function(data){
			...
			data.estiItemList.sort((a,b)=>a.estiNo-b.estiNo || a.dspNo - b.dspNo || parseFloat(a.estiSeq) - parseFloat(b.estiSeq)); //2024.06.18 sg
			...
			if (data.estiItemList.length == 0){
				...
				for(i=0;i<data.estiItemList.length;i++){
				    ...
				    list.push({
						...
						,className: data.estiItemList[i].className  //20240724 supi 구분, 공장번호 추가
						,factoryNo: data.estiItemList[i].factoryNo
						,specialNote: data.estiItemList[i].specialNote   //특이사항. 20241014 sg
...
function stockCheck(){
	...
	*/
	...
	$.fancybox.open({
	  ...
	  href : '/order/stock-check-up?estiNo='+estiNo    , // 불러 올 주소
	  type : 'iframe',
	  width : '90%',
	  height : '90%',
	  padding :0,
...
function divide() {
	...
	for (var i = 0, len = checkedItems.length; i < len; i++) {
		rowItem = checkedItems[i];		
		estiSeq = rowItem.item.estiSeq
		if (estiSeq === undefined ){
	...
	for (var i = 0, len = checkedItems.length; i < len; i++) {
		...
		var item = new Object();
	//rowList[i] = {
		//item.estiSeq = rowItem.item.estiSeq+'-분할', //rowItem.item.estiSeq, 
		item.estiSeq = rowItem.item.estiSeq+'.분할', //2023.07.03 그리드정렬떄문에 변경
	     ...
	     var options = {
	    	success : function(data) {
				...
				for(i=0;i<data.estiItemList.length;i++){
					...
					AUIGrid.addRow(myGridID, item, i);
				}
				*/
				var item;
				var rowList = [];
				...
				for(i=0;i<data.estiItemList.length;i++){
					...
					//console.log("dcExceptYN:"+dcExceptYN)
					var item = new Object();
					...
					rowList[i] = {
					    ...
					    salePrice : data.estiItemList[i].cnt * data.estiItemList[i].unitPrice
					    //sumPrice : data.estiItemList[i].cnt * data.estiItemList[i].unitPrice
					    */
					}
				}
				...
				*/
	            ...
	            firstGrid.appendToList([{},{},{}]);
          		...
          		for(ix=0;ix<data.orderSubList.length;ix++){
          			firstGrid.setValue(ix, "itemCode", data.orderSubList[ix].itemCode);
          			firstGrid.setValue(ix, "Qty", data.orderSubList[ix].qty);  
          			firstGrid.setValue(ix, "SubRemark", data.orderSubList[ix].subRemark);  
          			firstGrid.setValue(ix, "custMemo", data.orderSubList[ix].custMemo);
...
$("#btnDownload").click(function() {
	...
	//window.location.href = "/order/esti-up-print?estiNo="+estiNo+"&memoYN="+printMemoYN+"&imgYN="+imgYN +"&printDcYN=" + printDcYN;
	//window.location.href = "/order/esti-up-print?estiNo="+ $("#estiNo").val();
	//var url = "/order/esti-up-print?estiNo="+estiNo+"&memoYN="+printMemoYN+"&imgYN="+imgYN +"&printDcYN=" + printDcYN;
...
function updateGridRowCustMgr(obj,name) {
	...
	//$("#consignCustCode").val(rowItem.custCode);
	//$("#consignCustName").val(rowItem.custName);
	var phone = rowItem.phone1;
	if (phone == '') {
		phone = rowItem.phone2;
	}
	$(obj).val(rowItem.name);
	$("#"+name+"").val(phone);
	
	var dialogCust;
	dialogCust = $( "#dialog-form-custMgr");			
	dialogCust.dialog("close");
...
function updateGridRow() {
	...
	item = {
		...
		,className : rowItem.className  //20240724 supi 구분, 공장번호// 품번돋보기로 다이얼로그창에서 부품 선택한후 우측 하단'확인'버튼 누를때
...
//2023.08.25 100단위올림처리
//$("#btnCeil").click(function(){
//	fn_dcProc('', -1, 'Y', 100);
	/*
	var allItems = AUIGrid.getGridData(myGridID);
	var rowIdField = AUIGrid.getProp(myGridID, "rowIdField");
	...
	for (var i = 0, len = allItems.length; i < len; i++) {
		rowItem = allItems[i];
		rowId = rowItem[rowIdField];
		salePrice = AUIGrid.getCellValue(myGridID, i, "salePrice");
