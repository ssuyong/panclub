...
$(document).ready(function(){
	...
	if(document.location.hash){
        ...
        findDataToServer("/logis/rl-dtl-list",page);
  	}
	...
	$("#btnFind").click(function(){
		...
		document.location.hash = '#info'+currentPage+"!"+rlNo_val+"!"+sYmd+"!"+eYmd;
		
		findDataToServer("/logis/rl-dtl-list", 1);
	});
...
function createAUIGrid() {
	...
	var columnLayout = [		 
		...
		,{ dataField : "rlSeq",      headerText : "출고순번", width : 100, editable : false }
		,{ dataField : "rlReqYmd",      headerText : "요청일자*", width : 100, editable : false }
		...
		,{ dataField : "reqCnt",      headerText : "요청수량", width : 120, editable : false   }
		,{ dataField : "rlCnt",     headerText : "출고수량" }
		,{ dataField : "rlUnitPrice",     headerText : "출고가", editable : false }
		,{ dataField : "rlSumPrice",     headerText : "합계", editable : false }
		,{ dataField : "reqㅂRegUserId",     headerText : "요청자", editable : false }
		,{ dataField : "rlRegUserId",     headerText : "처리자" , editable : false}
			...
			if (data.rlItemList.length == 0){
				...
				for(i=0;i<data.rlItemList.length;i++){
				    list.push({						  
						...
						,rlReqYmd: data.rlItemList[i].rlReqYmd  
						,rlReqNo: data.rlItemList[i].rlReqNo 
						,custOrderNo: data.rlItemList[i].custOrderNo 
						...
						,reqCnt: data.rlItemList[i].reqCnt 
