...
$(document).ready(function(){
	...
	if(document.location.hash){
        ...
        findDataToServer("/logis/ri-req-list",page);
  	}
	...
	$("#btnFind").click(function(){
		...
		findDataToServer("/logis/ri-req-list", 1);
	});
...
// 칼럼 레이아웃 작성
var columnLayout = [ 
	{ dataField : "gvComName",   headerText : "요청업체명" , width: 100 } // 프로시저에서 반환받아서 쓰세요
	 ,{ dataField : "dmdYmd",    headerText : "요청일", width : 100} 
	,{ dataField : "riReqNo",     headerText : "요청번호", width : 100,
	...
	,{ dataField : "carNo",   headerText : "차량정보" }
	,{ dataField : "riSumPrice",     headerText : "공급가액", dataType: "numeric",formatString: "#,##0"  , style:"right" , width : 150  } //20230710 bk
	,{ dataField : "riTaxPrice",     headerText : "세액" , dataType: "numeric",formatString: "#,##0"  , style:"right", width : 150  } //20230710 bk
	,{ dataField : "riSumPriceTax",     headerText : "합계" , dataType: "numeric",formatString: "#,##0"  , style:"right", width : 150  } //20230710 bk
	...
	,{ dataField : "receiver",     headerText : "수신자", width : 80  }
	,{ dataField : "procStatus",      headerText : "처리내역"}
	,{ dataField : "gvComCode",   headerText : "요청업체코드" , width: 100 ,visible: false}//2023.09.21 yoonsang
	
];
//2023.07.10 bk
 var footerLayout = [
		{		labelText: "∑",		positionField: "#base",		style: "aui-grid-my-column"	}, 
		{		dataField: "riSumPrice",		positionField: "riSumPrice",		operation: "SUM",		formatString: "#,##0"	}, 
		{		dataField: "riTaxPrice",		positionField: "riTaxPrice",		operation: "SUM",		formatString: "#,##0"	}, 
		{		dataField: "riSumPriceTax",		positionField: "riSumPriceTax",		operation: "SUM",		formatString: "#,##0"	}, 
...
function createAUIGrid(columnLayout) {
	...
	AUIGrid.bind(myGridID, "cellDoubleClick", function (event) {
		...
		var riReqNo = event.item.riReqNo;
		var gvComCode = event.item.gvComCode;
		//console.log("storageUseReqNo:"+storageUseReqNo);  
		if (event.dataField == 'riReqNo') {   
			$.fancybox.open({
			  href : '/logis/ri-req-item-list?riReqNo='+riReqNo  + '&gvComCode=' + gvComCode  , // 불러 올 주소
			  type : 'iframe',
			  width : '90%',
			  height : '90%',
			  padding :0,
			...
			if (data.reqList.length == 0){
				...
				for(i=0;i<data.reqList.length;i++){
					list.push({
						...
						,riSumPrice :data.reqList[i].riSumPrice //2023.07.10 bk
						,riTaxPrice :data.reqList[i].riTaxPrice //2023.07.10 bk
						,riSumPriceTax :data.reqList[i].riSumPriceTax //2023.07.10 bk
						,gvComCode :data.reqList[i].gvComCode //2023.09.21 yoonsang
