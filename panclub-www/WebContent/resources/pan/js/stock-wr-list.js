...
$(document).ready(function(){
	...
	//반출번호가  존재하는 경우 
	let stockYm = $("#stockYm").text();
	if (stockYm !=''){		
		findReqItem('/logis/stock-wr-list');
	}	  
...
function createAUIGrid() {
	...
	// AUIGrid 칼럼 설정
	var columnLayout = [		 
		 { dataField : "wrType",      headerText : "입/출고", width : 100, editable : false }
		,{ dataField : "orderGroupId",      headerText : "주문그룹ID", width : 100, editable : false }
		,{ dataField : "whNo",      headerText : "입고번호", width : 100, editable : false }
		,{ dataField : "plCustCode",      headerText : "발주처코드", width : 100, editable : false }
		,{ dataField : "plCustName",      headerText : "발주처명*", width : 100, editable : false }
		,{ dataField : "storUseReqNo",      headerText : "창고사용요청번호", width : 100, editable : false }
		,{ dataField : "suCustCode",      headerText : "납품처코드", width : 100, editable : false }
		,{ dataField : "suCustName",      headerText : "납품처명", width : 140, editable : false }
		,{ dataField : "whQty",      headerText : "입고수량", width : 140, editable : false} 
		,{ dataField : "rlQty", headerText : "출고수량", width: 120, editable : false  } 
		,{ dataField : "rackCode", headerText : "랙위치코드", width: 120, editable : false  }
		,{ dataField : "rackName",      headerText : "랙위치명", width : 120, editable : false   }
		,{ dataField : "stockQty",     headerText : "재고" }
		,{ dataField : "regYmd",     headerText : "발생일", editable : false }
	];
...
function findReqItem(url) {
	//console.log("initem");
	var list = [];
	var stockYm = $("#stockYm").text();
	var storCode = $("#storCode").text();
			...
			if (data.stockWrList.length == 0){
				...
				for(i=0;i<data.stockWrList.length;i++){
				    list.push({						  
						...
						,orderGroupId: data.stockWrList[i].orderGroupId  
						,whNo: data.stockWrList[i].whNo  
						,plCustCode: data.stockWrList[i].plCustCode 
						,plCustName: data.stockWrList[i].plCustName 
						,storUseReqNo: data.stockWrList[i].storUseReqNo 
						,suCustCode: data.stockWrList[i].suCustCode 
						,suCustName: data.stockWrList[i].suCustName 
						,whQty: data.stockWrList[i].whQty 
						,rlQty: data.stockWrList[i].rlQty
						,rackCode: data.stockWrList[i].rackCode 
						,rackName: data.stockWrList[i].rackName 
						,stockQty: data.stockWrList[i].stockQty
						,regYmd: data.stockWrList[i].regYmd 
					});
