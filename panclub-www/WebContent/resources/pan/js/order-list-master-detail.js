...
var columnLayout_detail = [		 
    ...
    ,{ dataField : "dspNo",      headerText : "노출순서", width : 56 }	 
	...
	,{ dataField : "itemName", headerText : "품명", width: 160  } 
	...
	,{ dataField : "importPrice",     headerText : "수입가", visible : false }
	...
	,{ headerText : "발주예상", 
		children: [
			...
			,{ dataField : "placeCustName",      headerText : "발주처명" } 
			,{ dataField : "placeUnitPrice",      headerText : "예정발주단가", dataType: "numeric",formatString: "#,##0" , style:"right"  }
        ]
    }    
	,{ headerText : "실제발주", 
		children: [
...
function createAUIGrid(columnLayout) {
	...
	AUIGrid.bind(myGridID, "cellDoubleClick", function (event) {
		...
		if (event.dataField == "orderNo") {  
		...
		}else if (event.dataField == "orderGroupId") {  
			...
			findOrderItem('/order/order-item-list',event.item.orderNo);
		}		
...
function createAUIGrid_detail(columnLayout) {
	...
	AUIGrid.setFooter(myGridID_detail, footerLayout_detail);
  	...
  	AUIGrid.bind(myGridID_detail, "cellDoubleClick", function( event ) {
	    ...
	    if (event.dataField == "estiNo") {   
		 ...
		 if (event.dataField == 'pcReqNo'){
			...
			window.open('/order/pc-req-item-list?pcReqNo='+pcReqNo, '_blank');	
		}
	});
	
};

function findOrderItem(url,orderNo) {
	var list=[];
	...
	$.ajax(	{
		...
		dataType : "json",
		data: {
			"workingType": "ONE",  //2024.10.08 hsg
			"orderNo":orderNo
