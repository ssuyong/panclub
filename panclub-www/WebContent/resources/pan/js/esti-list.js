...
$(document).ready(function(){
	...
	$(document).ready(function() {
		$('#supplyCustCode').select2();
	});
	*/
	
	findSrchCode2("/base/cust-list", 1) // 거래처
...
var columnLayout = [ 
	...
	,{ dataField : "orderTotalPrice",      headerText : "주문금액", dataType: "numeric",formatString: "#,##0"  , style:"right"}
...
function createAUIGrid(columnLayout) {
	...
	AUIGrid.bind(myGridID, "cellDoubleClick", function (event) {
		...
		if (event.columnIndex == 1) {   
			...
			document.location.hash = '#info'+currentPage+"!"+estiNo_val+"!"+sYmd+"!"+eYmd+"!"+custCode_val+"!"+supplyCustCode_val+"!"+ymdIgnoreYN;				
			//document.location.hash = '#info'+currentPage+"!"+estiNo_val+"!"+sYmd+"!"+eYmd+"!"+custCode_val+"!"+supplyCustCode_val;
	    /* 	
			...
			if (data.estiList.length == 0){
				...
				for(i=0;i<data.estiList.length;i++){
					list.push({
						...
						,orderTotalPrice: data.estiList[i].orderTotalPrice
				...
				for (i = 0; i < data.custList.length; i++) {
					if (type == 1) {
						if (data.custList[i].custType == "C3") {
							...
							$("#supplyCustCode").append("<option value='" + custCode + "' >" + custName + "</option>");
						}

					}if (type == 4) {
						if (data.custList[i].custType == "C5") {
							custCode = data.custList[i].custCode;
							custName = data.custList[i].custName;
						...
						if (data.custList[i].custType == "C1") {
							custCode = data.custList[i].custCode;
							custName = data.custList[i].custName;
							list[j] = { "code": custCode, "value": custName };
							j = j + 1;
