...
function clTypeProc(clType) {
	...
	for (var i = 0, len = checkedItems.length; i < len; i++) {
		...
		if (rowItem.item.clReqYN == "Y" && rowItem.item.clReqChkYN == "" ) { err1 = "Y"; break;}
	...
	if(err1 == "Y"){alert("이미 청구요청된 품목은 청구구분을 변경할 수 없습니다!"); return;}
...
//발주요청 - 대량데이터. 2023.08.04
$('#placeReqBig').on("click", function (e) {
    ...
    $.ajax({
	      type: "POST",
	      cache: false,
	      url: '/order/place-req-up-big', // preview.php
	      data: {
		        	orderGroupId : orderGroupId
		        	,ordArr : ordArr
		        	,seqArr : seqArr
...
/*
// Attach Fancybox to the form submission button
  $("#placeReqBig").on("click", function() {
    $.fancybox.open({
      src: "#myForm",
      type: "inline",
      opts: {
        afterClose: function() {
          // Clear the form after closing Fancybox
          $("#myForm")[0].reset();
        }
      }
    });
...
// Handle form submission using AJAX
  $("#myForm").on("submit", function(event) {
	    ...
	    $.ajax({
		      type: "POST",
		      url: '/order/place-req-up', // Replace with your server-side endpoint
		      data: {
			        	orderGroupId : orderGroupId
			       },
		      success: function(response) {
			        ...
			        // Close Fancybox after successful submission
			        $.fancybox.close();
		      },
...
function placeReqBig() {
	...
	//alert(part_seq_array_value);
	 $.ajax({
	       type: "POST",
	       cache: false,
	       url: '/order/place-req-up', // this.href, // preview.php
...
$('.preview2').on("click", function (e) {
    ...
    $.ajax({
      type: "POST",
      cache: false,
      url: '/order/place-req-up', // preview.php
      data: {
	        	orderGroupId : '20230804002' // all form fields
	       } 	,
      success: function (data) {
        // on success, post (preview) returned data in fancybox
        $.fancybox(data, {
