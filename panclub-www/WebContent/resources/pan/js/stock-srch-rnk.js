...
$(document).ready(function(){
	...
	$("#btnFind").click(function(){
		findDataToServer("stock-srch-rnk");
	});
});
...
var columnLayout = [ 
	...
	,{
		headerText: "검색수",
			children: [
				  { dataField : "allSrchCnt",  headerText : "전체", width : 100 ,dataType: "numeric" ,formatString: "#,##0"  }	
				 ,{ dataField : "totalRank",  headerText : "순위", width : 100 ,dataType: "numeric" ,formatString: "#,##0"  }	
				 ,{ dataField : "exSrchCnt",  headerText : "외부업체", width : 100 ,dataType: "numeric" ,formatString: "#,##0"  }	
				 ,{ dataField : "custRank",  headerText : "순위", width : 100 ,dataType: "numeric" ,formatString: "#,##0"  }	
			]
	}		
	,{
		headerText: "검색업체수",
			children: [
				  { dataField : "allCustQty",  headerText : "전체", width : 100 ,dataType: "numeric" ,formatString: "#,##0"  }	
				 ,{ dataField : "exCustQty",  headerText : "외부업체", width : 100 ,dataType: "numeric" ,formatString: "#,##0"  }	
			]
	}		
...
var footerLayout = [{
		...
		dataField: "allSrchCnt",
		positionField: "allSrchCnt",
		operation: "SUM",
		formatString: "#,##0"
		,style:"right"
	}, {
		dataField: "exSrchCnt",
		positionField: "exSrchCnt",
		operation: "SUM",
		formatString: "#,##0"
		,style:"right"
	}, {
		dataField: "allCustQty",
		positionField: "allCustQty",
		operation: "SUM",
		formatString: "#,##0"
		,style:"right"
	}, {
		dataField: "exCustQty",
		positionField: "exCustQty",
		operation: "SUM",
		formatString: "#,##0"
		,style:"right"
	}
...
function findDataToServer(url) {
	...
	var startYmSerch = document.getElementById("startpicker-input").value;
	var endYmSerch = document.getElementById("endpicker-input").value; 
	...
	*/
	...
	$.ajax(	{
		...
		dataType : "json",
		data: {
			 workingType,serchItem,startYmSerch,endYmSerch
		},
		async: false,
		 traditional: true,
		...
		success:function(data){	 
			...
			if(data.stockSrchRnk.length ==0)		{
				...
				for(i=0;i<data.stockSrchRnk.length;i++){				
					...
					list.push({
						makerCode: data.stockSrchRnk[i].makerCode  
						,itemNo: data.stockSrchRnk[i].itemNo 
						,itemName: data.stockSrchRnk[i].itemName 
						,allSrchCnt: data.stockSrchRnk[i].allSrchCnt
						,totalRank: data.stockSrchRnk[i].totalRank
						,exSrchCnt: data.stockSrchRnk[i].exSrchCnt
						,custRank: data.stockSrchRnk[i].custRank
						,allCustQty: data.stockSrchRnk[i].allCustQty						
						,exCustQty: data.stockSrchRnk[i].exCustQty
					});
				}		
