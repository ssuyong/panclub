...
$(document).ready(function(){
	...
	if(document.location.hash){
        ...
        findDataToServer("/logis/wh-dtl-list",page);
  	}
	...
	$("#btnFind").click(function(){
		...
		document.location.hash = '#info'+currentPage+"!"+whNo_val+"!"+sYmd+"!"+eYmd+"!"+itemId_val+"!"+itemNo_val+"!"+orderGroupId_val+"!"+custOrderNo_val ;
		
		findDataToServer("/logis/wh-dtl-list", 1);
	});
...
function createAUIGrid() {
	...
	var columnLayout = [
		...
		,{ dataField : "whSeq",      headerText : "입고순번", width : 100, editable : false ,visible: false}
		,{ dataField : "whYmd",      headerText : "입고일자", width : 100, editable : false }
		,{ dataField : "placeRlYmd",      headerText : "발주처출고일자", width : 100, editable : false }
		,{ dataField : "placeCustCode",      headerText : "발주처코드", width : 100, editable : false ,visible: false}
		,{ dataField : "placeCustName",      headerText : "발주처", width : 100, editable : false }
		,{ dataField : "placeYmd",      headerText : "발주일자", width : 100, editable : false ,visible: false }
		...
		,{ dataField : "custOrderNo",      headerText : "거래처오더번호", width : 120, editable : false }
		,{ dataField : "orderGroupId",      headerText : "주문그룹ID", width : 100, editable : false ,
		styleFunction: function (rowIndex, columnIndex, value, headerText, item, dataField) { if (value != "0") {	return "auigrid-color-style-link";	}	return null;	}}
		//,{ dataField : "makerCode",      headerText : "제조사", width : 100, editable : false }
		...
		,{ dataField : "whUnitPrice",     headerText : "입고가",width: 100,dataType: "numeric",formatString: "#,##0"  , style:"right" , editable : false }
		,{ dataField : "whSumPrice",     headerText : "합계", width: 100 , dataType: "numeric",formatString: "#,##0"  , style:"right" , editable : false }
		,{ dataField : "placeRegUserId",     headerText : "요청자", width: 80, editable : false }
		,{ dataField : "whRegUserId",     headerText : "처리자" , width: 80 , editable : false}
		///,{ dataField : "payStatus",     headerText : "결제상태" , editable : false}
		//,{ dataField : "withdrawStatus",     headerText : "출금요청여부" , editable : false}
		...
		success:function(data){
			...
			if (data.whList.length == 0){
				...
				for(i=0;i<data.whList.length;i++){
					...
					regUserId = data.whList[i].regUserId;
...
function findDataToServer(url) {
   ...
   // console.log("a:"+seqArr);
	...
	if ($('#ymdIgnoreYN ').is(':checked') == true){
   	...
   	if ($('#ymdIgnoreYN').is(':checked') == true){
	   if ((!itemNo || itemNo.trim() == '' ) && (!whNo || whNo.trim() == '')
			...
			if (data.whItemList.length == 0){
				...
				for(i=0;i<data.whItemList.length;i++){
				    list.push({
						...
						,placeRlYmd: data.whItemList[i].placeRlYmd
						,placeCustCode: data.whItemList[i].placeCustCode
						,placeCustName: data.whItemList[i].placeCustName
						
						,makerName: data.whItemList[i].makerName //231102
...
function changePlRlYmd(){
	var whNoArr = $("#popWhNoArr").val();
	var whSeqArr = $("#popWhSeqArr").val();
	var placeRlYmd = $("#popPlRlYmd").val();
	var workingType = "CHANGE_PL_RL_DATE"
	...
	data.placeRlYmd = placeRlYmd;
	data.workingType = workingType;
	
	$.ajax({
	    url : "/logis/changePlRlYmd",
	    dataType : "json",
	    type : "POST",
	    //contentType: "application/json; charset=utf-8",
	    contentType : "application/x-www-form-urlencoded;charset=UTF-8",
