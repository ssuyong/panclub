...
$(document).ready(function(){
	$("#itemSelectBox").css('max-height' , window.innerHeight-70); // 레이어팝업 최대 크기 지정
	findConsignCode();
}) 
...
$('#btn1').on('click', function(e) { 
	...
	const barcodeInput = $("#m_itemNo").val();
	let items = null; 
		...
		{ 
			simplePostAjax('/logis/barcodeItem' , //idx타입으로 idx를 보내서 해당 정보 받음
							...
							(result)=>{
								...
								items = result.item; 
								$("#m_itemNo").val(result.item['itemNo']);
							}) 
	...
	else 
	{
		if(items.length>1) //복수의 결과값
		{
			...
			const itemSelectBoxDiv = $("#itemSelectBox");
...
function setItemInfo(item)
{
	...
	scanItemInfo = item;
	$("#m_basketTable_itemNo").html(item.itemNo);
	$("#m_basketTable_itemInfo").html(`${item.makerCode}|${item.itemName||item.itemNameEn}`); 
...
{
	const itemTable  = $("#m_basketItemTable_Header");
	const now = item.modified?new Date(item.modified) :new Date;  //처리번호 조회로 불러와서 추가된 부품의 경우 데이터에 기록된 수정날짜를 일시로 받음 이외엔 현재시간
	
	itemTable.after(`<tr class="m_basketTableItem" id="m_basketTable_Header1">
					<td class="m_bt0" scope="col"><input type="checkbox" name="chkRow"></th>
					<td class="m_bt1 m_bt_textalign_left font13p" scope="col">${item.itemNo.toString()}<br>${item.makerCode}|${item.itemName||item.itemNameEn||''}</th> 
					<td class="m_bt3 font13p" scope="col">${now.getFullYear().toString().substring(2,4)}:${(now.getMonth()+1).toString().padStart(2,'0')}:${(now.getDate()).toString().padStart(2,'0')}<br>
												  ${now.getHours().toString().padStart(2,'0')}:${now.getMinutes().toString().padStart(2,'0')}:${now.getSeconds().toString().padStart(2,'0')}</th>
					
				</tr>`);
	
	$("#m_basketCount_Label2").html((parseInt($("#m_basketCount_Label2").html())+1).toString());
...
$("#baskerRowDelete").on('click',()=>{
	checkDeleteRow();
})
$("#listUpFindBtn").on('click',()=>{
	findListUpMst();
})
$("#consignFindBtn").on('click',()=>{ 
...
function checkDeleteRow ()
{
	...
	{
		$("#m_basketCount_Label2").html((parseInt($("#m_basketCount_Label2").html())-rows.length).toString()); 
		for(row of rows)
		{ 
		...
		{   
			$(`#itemTable>tbody>tr:nth-child(${row+2})`).remove();
					...
					{"workingType":"LIST",
					 ...
					 (result)=>{
						...
						if(result.stockWrList[0].consignComCode)	
					 		$("#consignComInfo").val((result.stockWrList[0].consignComName||'')+'|'+(result.stockWrList[0].consignComCode||''))
						$(".m_basketTableItem").remove();
						$("#m_basketCount_Label2").html('0');
						selectWrNo = wrNo;
...
function setCustInfo(custCode,custName)
{
	...
	$("#consignComInfo").val(`${custName}|${custCode}`);
}
...
//
	...
	if($("#consignComInfo").val())
	{
		consignComCode = $("#consignComInfo").val().split('|')[1];
	}
...
//	else data.stockWrItemUpdate = [];
	...
	$.ajax({
	    ...
	    success: function(data) { 
			deleteItemList = [];
			$("#itemListNo").val(data.wrNo);	
			alert($("#saveBtn").html()+'에 성공하였습니다.\n처리번호 : '+data.wrNo);
			findListUpMst(true);
