...
/*function downImg() {
  ...
  let page = 1;
  let orderNo = '';
   orderNo = "주문서_"+document.getElementById('printOrderNo').value;
      //estiNo = "견적서_";
  while (position < wrapper.offsetHeight) {
    ...
    context.translate(-horizontalPosition, -position);
...
function downImg() {
  ...
  let page = 1;
  let estiNo = '';
   estiNo = "주문서_"+document.getElementById('printOrderNo').value;
      //estiNo = "견적서_";
  while (position < wrapper.offsetHeight) {
    // Create a canvas element for each page
    const canvas = document.createElement('canvas');
 ...
 //   context.translate(0, -position);
    ...
    html2canvas(wrapper, { canvas: canvas }).then(function(canvas) {
      ...
      //link.download = `견적서_'+estiNo+'${page}.png`;
      link.download = estiNo+`_${page}.png`;
