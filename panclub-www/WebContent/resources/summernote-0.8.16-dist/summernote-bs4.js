    ...
    value: function insertImage(src, param) {
      ...
      return createImage(src, param).then(function ($image) {
        ...
        if (typeof param === 'function') {
          ...
          //2020.03.20 sgham - 모바일 등 다른 기기에서 이미지가 크게 보이는 것 해결하기 위해서 수정함
          //$image.css('width', Math.min(_this3.$editable.width(), $image.width()));
          if ($image.width() > 300) { //이미지 사이트 이것보다 작은 경우에는 원본크기로 보이도록
        	  $image.css('width',( Math.min(_this3.$editable.width(), $image.width()) / _this3.$editable.width() )*100 +'%');
          }else{
        	  $image.css('width', Math.min($image.width()));
          }
...
function () {
  ...
  ImageDialog_createClass(ImageDialog, [{
    ...
    value: function initialize() {
      ...
      var $container = this.options.dialogsInBody ? this.$body : this.options.container;
      var body = ['<div class="form-group note-form-group note-group-select-from-files">','<script type="text/javascript" src="${pageContext.request.contextPath}/resources/js_user/spin.js"></script>', '<div class="spinner  v-bs4" id="spinner"></div>','<label for="note-dialog-image-file-' + this.options.id + '" class="note-form-label">' + this.lang.image.selectFromFiles + '</label>', '<input id="note-dialog-image-file-' + this.options.id + '" class="note-image-input form-control-file note-form-control note-input" ', ' type="file" name="files" accept="image/*" multiple="multiple"/>', imageLimitation, '</div>', '<div class="form-group note-group-image-url">', '<label for="note-dialog-image-url-' + this.options.id + '" class="note-form-label">' + this.lang.image.url + '</label>', '<input id="note-dialog-image-url-' + this.options.id + '" class="note-image-url form-control note-form-control note-input" type="text"/>', '</div>'].join('');
